...
# Graficar un gráfico de barras en blanco y negro (sin blanco en el fondo)
ggplot(tabla_predicciones, aes(x = Prediccion, fill = Prediccion)) +
  geom_bar(stat = "count", show.legend = FALSE) +
  scale_fill_manual(values = c("Bueno" = "black", "Malo" = "gray")) +  # Colores en tonos de gris
  ...
  theme_minimal(base_size = 12) +  # Estilo minimalista
  theme(
    plot.title = element_text(hjust = 0.5, color = "black"), 
    plot.subtitle = element_text(hjust = 0.5, color = "black"),
    axis.text = element_text(color = "black"), 
    axis.title = element_text(color = "black"),
    panel.background = element_rect(fill = "gray40", color = "gray40"),  # Fondo oscuro (gris muy oscuro)
    plot.background = element_rect(fill = "gray40", color = "gray40"),  # Fondo oscuro para todo el gráfico
    panel.grid.major = element_line(color = "gray80", linewidth = 0.5),  # Líneas de cuadrícula mayores en gris claro
    panel.grid.minor = element_line(color = "gray60", linewidth = 0.25)   # Líneas de cuadrícula menores en gris intermedio
  )
